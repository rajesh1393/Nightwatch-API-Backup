...
for (int index =0; index<data.getRowNumbers(); index++) {
	...
	println requestContent.binaries_ingressors[0].content_type
	println requestContent.binaries_ingressors[0].state
	...
	println (" Response for the Binaries with Content type: ${ requestContentFinal.binaries_ingressors[0].content_type}")
	println (" Response for the Binaries with state: ${requestContentFinal.binaries_ingressors[0].state}")
	println (" Response for the Binaries : ${requestContentFinal}")
	'Verify the Binary_Ingressor State'
	WS.verifyElementPropertyValue(response,"binaries_ingressors[0].state",'succeeded', FailureHandling.STOP_ON_FAILURE)
