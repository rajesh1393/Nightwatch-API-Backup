...
for (int index =0; index<data.getRowNumbers(); index++) {
	...
	readBinaryIngressor.setRestUrl('http://us-west-2.api.acceptance.unimatrix.io/realms/f53dbe250d8a05d1ca44b28b95eb5e20/binaries/'+internal.GlobalVariable.binaryReadId+'/ingressors?uuid.in='+internal.GlobalVariable.binaryIngressId+'&access_token=5422e9fefd05e874dfea631770b9f7bb')
	...
	println ("Response for Binary_Ingressor with Content Type: ${requestContent.binary_ingressors[0].content_type}")
	println ("Response for Binary_Ingressor with State: ${requestContent.binary_ingressors[0].state}")
	...
	println ("Response for Binary_Ingressor with Content Type: ${requestContentFinal.binary_ingressors[0].content_type}")
	println ("Response for Binary_Ingressor with state: ${requestContentFinal.binary_ingressors[0].state}")
